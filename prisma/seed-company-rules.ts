...
async function main() {
  console.log("Backfilling company rules...\n");

  let seeded = 0;
  for (const rule of COMPANY_RULES_SEED) {
    await prisma.companyRule.upsert({
      where: { ruleId: rule.ruleId },
      update: {}, // We don't overwrite if it exists, to not destroy custom modifications, or we could update
      create: {
        ruleId: rule.ruleId,
        name: rule.name,
  ...
  console.log(`\nBackfilled ${seeded} company rules.`);
}

main()
  .catch((error) => {
    console.error("Company rules backfill failed:", error);
    process.exit(1);
  })
  .finally(async () => {
